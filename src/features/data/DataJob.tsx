...
import { fetchDataJobExperience } from "../../api/client";
import styles from "./stylesheet/DataJob.module.css";
  ...
  useEffect(() => {
    async function loadJobs() {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchDataJobExperience();
  ...
  return (
    <section className={styles.jobSection}>
      ...
      {!loading && !error && jobs.length === 0 && (
        <p className={styles.stateText}>No roles added yet.</p>
      )}
          ...
          <article key={job.id} className={styles.jobCard}>
              ...
              <div className={styles.companyInfo}>
                ...
                <div>
                  ...
                  <div className={styles.metaRow}>
                    {job.employment_type && <span className={styles.metaBadge}>{job.employment_type}</span>}
                    {job.department && <span className={styles.metaBadge}>{job.department}</span>}
                  </div>
                </div>
              </div>
              ...
              <div className={styles.column}>
                {job.responsibilities?.length > 0 && (
                  <div className={styles.sectionBlock}>
                    <span className={styles.sectionLabel}>Responsibilities</span>
                    ...
                    <div className={styles.metricsList}>
                      {job.impact_metrics.map((m: any, i: number) => (
                        <div key={i} className={styles.metricItem}>
                          ...
                          {m.description && <div className={styles.metricDescription}>{m.description}</div>}
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className={styles.footerRow}>
              ...
              <div className={styles.linksRow}>
                {job.company_website && <a className={styles.link} href={job.company_website} target="_blank" rel="noreferrer">Website</a>}
                {job.reference_letter_url && <a className={styles.link} href={job.reference_letter_url} target="_blank" rel="noreferrer">Reference</a>}
                {job.certificate_url && <a className={styles.link} href={job.certificate_url} target="_blank" rel="noreferrer">Certificate</a>}
              </div>
            </div>
          </article>
        ))}
