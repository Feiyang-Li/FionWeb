...
export default function FinanceProject() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const data = await fetchFinanceProjects();
        data.sort((a: any, b: any) => new Date(b.start_date || 0).getTime() - new Date(a.start_date || 0).getTime());
        setProjects(data);
      } catch (err: any) {
        console.error(err);
  ...
  if (loading) return <div className={styles.loading}>Loading finance projects...</div>;
  ...
  const getDescriptionSnippet = (markdown?: string, length = 220) => {
    if (!markdown) return "";
    const plain = markdown.replace(/[#*_`>-]/g, "").replace(/\[(.*?)\]\(.*?\)/g, "$1");
            ...
            <div className={styles.cardBody}>
              <header className={styles.cardHeader}>
                ...
                <div className={styles.projectMeta}>
                  {project.status && <span className={`${styles.statusBadge} ${project.status === "active" ? styles.statusActive : styles.statusOther}`}>{project.status}</span>}
                  {project.start_date && <div className={styles.dateText}>{project.start_date}{project.end_date ? ` — ${project.end_date}` : ""}</div>}
                </div>
              </header>
              ...
              <footer className={styles.cardFooter}>
                {project.slug && <a className={styles.linkButtonGhost} href={`/finance/projects/${project.slug}`}>Project page</a>}
